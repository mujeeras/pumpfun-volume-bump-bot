...
let BUY_INTERVAL = process.env.BUY_INTERVAL || 3000; // Interval to check for new listings (in ms)
...
const buyToken = async (tokenAccount, price) => {
  try {
    increase_vol.start("Preparing transaction...");
    ...
    increase_vol.succeed("Transaction sent.");
    logInfo(`Transaction Signature: ${signature}`);
  } catch (error) {
    increase_vol.fail("Failed to send transaction.");
...
const startSniper = async () => {
  ...
  while (true) {
    try {
      increase_vol.start("Checking for new listings...");

      const listings = increase_vol.succeed("Listings fetched.");
      ...
      await new Promise((resolve) => setTimeout(resolve, BUY_INTERVAL));
    } catch (error) {
      increase_vol.fail("An error occurred.");
      logError(error.message);
    }
  }
};
